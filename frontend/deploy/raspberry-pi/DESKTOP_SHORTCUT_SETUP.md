...
Chromium으로 **React 키오스크**(`frontend` 앱의 `/kiosk` 등)를 전체 화면으로 띄운 뒤, 닫았을 때 바탕화면 아이콘으로 다시 실행하고 싶을 때의 절차입니다. 운영 기준 키오스크는 React이며, `frontend/deploy/raspberry-pi` 아래 스크립트·systemd 유닛을 사용합니다.
